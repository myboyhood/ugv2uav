...
complx_tracker::complx_tracker() {
    ...
    cameraMatrix.at<double>(0, 0) = 1206.89;//1206.89fx
    cameraMatrix.at<double>(0, 2) = 960;//960cx
    cameraMatrix.at<double>(1, 1) = 1206.89;//I have change from 678.88 to 1206.89
    cameraMatrix.at<double>(1, 2) = 540;//540cy
...
//    start_time = ros::Time::now();
    ...
    // |
    // y
    if (STRUCTURE_INDEX == 0) {//plane struct
        objectPts.emplace_back(-0.165, 0.1, 0);//blue
        objectPts.emplace_back(0.165, 0.02, 0);//green
        objectPts.emplace_back(-0.165, 0.02, 0);//red
        objectPts.emplace_back(0.165, 0.1, 0);//white
    } else if (STRUCTURE_INDEX == 1) {//quad struct
        objectPts.emplace_back(0, 0.08, -0.165);//blue
        objectPts.emplace_back(0, 0.08, 0.165);//green
        objectPts.emplace_back(-0.165, 0.08, 0);//red
        objectPts.emplace_back(0.165, 0.08, 0);//white
...
//    Eigen::Vector3d euler_angles(0,-0.4,0);
    ...
    transform_cam.pretranslate(Eigen::Vector3d(0,0,0.1));
